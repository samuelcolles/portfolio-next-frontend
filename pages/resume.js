  ...
  return <>
    <div className='background-box background-sky'>
      <article className='grid-1-col container'>
        ...
        {props.toolLists.map(list => <ToolList key={list.id} toolList={list} />)}
      </article>
    </div>
...
export const getStaticProps = async () => {
  ...
  const resToolLists = await fetch(process.env.STRAPI_BACKEND_URL + '/api/tool-lists?populate[0]=tools&populate[1]=tools.icon&populate[2]=tools.nestedtools&populate[3]=tools.nestedtools.icon')
